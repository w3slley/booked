<?php
	...
	if(empty($book_title) || empty($author_name) || empty($category) || empty($year) || empty($classification)){
		...
		$book_cover_url = $data->download_book_cover_url($book_title, $author_name); //This function gets the url from the bookcover from goodreads servers.
		$data->add_book_event($user_id, $book_title, $author_name, $category_id, $month_id, $year_id, $classification, $task_date, $hash_id, $book_cover_url);
		//The bookcover along with the book information is added to the database.
